...
class LidarPublisher:
    def __init__(self, node_name):
        ...
        self.brake_publisher = rospy.Publisher('/carla/ego_vehicle/brake', Float32, queue_size=10)
        self.id_publisher = rospy.Publisher('/carla/ego_vehicle/id', Int32, queue_size=10)
    ...
    def publish_brake_data(self, brake_value):
        brake_msg = Float32(data=brake_value)
        self.brake_publisher.publish(brake_msg)

    def publish_ego_car_id(self, car_id):
        car_id_msg = Int32(data=car_id)
        self.id_publisher.publish(car_id_msg)
...
class CarlaSimulation:
    ...
    def _parse_vehicle_wheel(self):
        ...
        # reverseCmd = jsButtons[self._reverse_idx]

        self._control.steer = steerCmd
        self._control.brake = brakeCmd
        self._control.throttle = throttleCmd
        # self._control.gear = reverseCmd

        # toggle = jsButtons[self._reverse_idx]

        self._control.hand_brake = bool(jsButtons[self._handbrake_idx])

        if bool(jsButtons[self._reverse_idx]):
    ...
    def spawn_ego_vehicle(self):
        ...
        # Set the relative location
        lidar_location = carla.Location(3, 0, 2.5)
    ...
    def set_synchronous_mode(self):
        ...
        setting.fixed_delta_seconds = 0.05  # In sync with the lidar's rotation frequency
    ...
    def run_simulation_loop(self):
        ...
        # updating Pygame's internal state
        ...
        if isinstance(self._control, carla.VehicleControl):
            ...
            brake_value = self._control.brake
            self.lidar_publisher.publish_brake_data(brake_value)

        ego_car_id = self.ego_vehicle.id
        self.lidar_publisher.publish_ego_car_id(ego_car_id)
